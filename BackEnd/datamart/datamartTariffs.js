...
export async function insertTariff(tariffData) {
	const query = `
			INSERT INTO tariffs (
					tariff_id,
					tariff_name,
					...
					${tariffData.tariff_id},
					'${tariffData.tariff_name}',
					${tariffData.tariff_price},
					'${tariffData.tariff_vendor_code || ''}',
					${tariffData.is_active !== undefined ? tariffData.is_active : true}
			)
	`
	const options = {
		method: 'POST',
	...
	return new Promise((resolve, reject) => {
		...
		const req = https.request(options, res => {
			...
			res.on('end', () => {
				try {
					console.log('Tariff inserted successfully:', data)
					resolve(data)
				} catch (error) {
					console.error('Error tariff parsing response:', error)
					reject(error)
				}
			})
		})
...
export async function truncateTariffsCascade() {
	const query = `TRUNCATE TABLE tariffs`
